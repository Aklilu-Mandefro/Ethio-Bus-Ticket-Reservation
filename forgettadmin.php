...
<div id="adlog">
<div id="welcome">
<h3 align="center" style="padding-top:5px;">Forget Password Form</h3>	
</div>
...
if (isset($_POST['change_password'])) 
	...
	$resulsasst = mysql_query("SELECT * FROM admin WHERE username = '$username'  and email='$firstname' and   password='$priv_password' ");
	...
	if(strlen($passw) <=2 ) {
	 echo '<div class="alert alert-dismissable alert-e">';
		die( '<strong>'."Password Must Be Grater Than Or Equal To 3 Characters !".'</strong>');
		echo '</div>';
	} 
	...
	echo '<div class="alert alert-dismissable alert-error"><strong>';
		...
		$chack = mysql_query("UPDATE admin SET password='$new_password' WHERE username = '$username' and email='$firstname'   ");
		...
		echo "".'<a href="adminlogin.php?selam_User=">'."Login Here".'</a>';
		echo '</strong></div>';
		}
...
if (isset($_POST['forget'])) 
	...
	$result = mysql_query("SELECT * FROM admin WHERE username = '$username' and email='$firstname' ");
...
echo'<meta content="3;adminlogin.php" http-equiv="refresh" />';
echo '</strong></div>';	
	}
...
<form id="form1" name="forgett" method="POST" action="forgettadmin.php" onsubmit="return validateForm()">
				 <table id="ttp" align="center" border="0"cellpadding = "5" cellspacing = "10">
	    <tr><td colspan="2">  <img height="67" width="100px" src="image/admin.png"/>  </td>   </tr>    
            ...
            <td colspan="2">
				   ...
				   <td></td>
            <td colspan="2">
                	<a href="adminlogin.php?Ha_User="><font color='white'>Back to Login?</a>
                </td>
            </tr>
			</table>
		</form>
